...
impl FileViewer {
    ...
    pub fn load_file_with_width(
        ...
        // Max characters per line to prevent hang on single-line files (e.g. FB2, minified JSON)
        ...
        for content in raw_lines {
            ...
            // Truncate extremely long lines to prevent exponential wrapping cost
            let (display_content, truncated) = if content_no_tabs.chars().count() > MAX_LINE_CHARS {
                let s: String = content_no_tabs.chars().take(MAX_LINE_CHARS).collect();
                (s, true)
            } else {
                (content_no_tabs, false)
            };

            if self.wrap_lines {
                // Wrap long lines
                let mut wrapped_lines = Self::wrap_line(&display_content, max_width);
                if truncated {
                    if let Some(last) = wrapped_lines.last_mut() {
                        last.push_str(" …[line truncated]");
                    }
                }
...
mod tests {
    ...
    #[test]
    fn test_load_file_with_extremely_long_line() {
        // Verify that a file containing a multi-million-char line loads quickly
        // and does not hang. The content should be truncated to MAX_LINE_CHARS.
        use std::io::Write;

        let dir = std::env::temp_dir();
        let path = dir.join("dtree_test_longline.txt");

        // Write a file with one very long line (2M chars) and one normal line
        {
            let mut f = std::fs::File::create(&path).expect("create temp file");
            let long_line = "word ".repeat(400_000); // 2M chars
            writeln!(f, "{}", long_line).unwrap();
            writeln!(f, "normal line").unwrap();
        }

        let mut viewer = FileViewer::new();
        let start = std::time::Instant::now();
        viewer
            .load_file_with_width(&path, Some(200), 10_000, false, "base16-ocean.dark")
            .expect("load_file_with_width should not fail");
        ...
        // At least one content line (the truncated long line) must be present
        assert!(!viewer.content.is_empty());

        // The truncated line indicator must appear
        assert!(
            viewer
                .content
                .iter()
                .any(|l| l.contains("[line truncated]")),
            "Expected truncation marker in content"
        );
