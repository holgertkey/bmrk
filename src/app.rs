...
pub struct App {
    ...
    /// When true, the app renders in compact inline mode (8 rows, no alternate screen)
...
impl App {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> Result<Option<PathBuf>> {
        // In compact mode, track state changes that require a switch to fullscreen.
        let show_files_before = self.show_files;
        ...
        if self.compact_mode {
            // 's' was pressed: file viewer toggled on — switch to fullscreen.
            if !show_files_before && self.show_files {
                return Ok(Some(PathBuf::from("SWITCH_FULLSCREEN:")));
            }
            // Search results stay in compact mode — rendered inline instead of fullscreen.
        }
...
mod tests {
    ...
    #[test]
    fn test_compact_mode_bookmark_create_does_not_switch_fullscreen() {
        let temp_dir = std::env::temp_dir().join("dtree_test_compact_bm");
        std::fs::create_dir_all(&temp_dir).unwrap();
        ...
        // Must NOT return SWITCH_FULLSCREEN:
        let switched = result
            .as_ref()
            .map(|p| p.to_str().unwrap_or("").starts_with("SWITCH_FULLSCREEN:"))
            .unwrap_or(false);
        assert!(
            !switched,
            "bookmark creation should not trigger fullscreen switch"
        );
    ...
    #[test]
    fn test_compact_mode_search_does_not_switch_fullscreen() {
        let temp_dir = std::env::temp_dir().join("dtree_test_compact_search");
        std::fs::create_dir_all(&temp_dir).unwrap();
        ...
        assert!(
            !switched,
            "entering search should not trigger fullscreen switch"
        );
        ...
        assert!(
            !switched,
            "search execution should not trigger fullscreen switch in compact mode"
        );
        assert!(
            app.search.show_results,
            "search results should be shown inline"
        );
    ...
    #[test]
    fn test_compact_mode_bookmark_select_does_not_switch_fullscreen() {
        let temp_dir = std::env::temp_dir().join("dtree_test_compact_bm2");
        std::fs::create_dir_all(&temp_dir).unwrap();
        ...
        assert!(
            !switched,
            "bookmark selection should not trigger fullscreen switch"
        );

        // Bookmark selection mode must be active
