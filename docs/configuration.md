...
# Maximum display length for file/directory names (middle-truncation)
# Names longer than this are shortened with "..." in the middle
# Example: "very_long_project_name.rs" -> "very_long_pro...name.rs"
...
Long filenames and directory names are middle-truncated for readability:

```toml
max_name_length = 30    # Default: truncate at 30 characters
max_name_length = 0     # Disable truncation (show full names)
max_name_length = 40    # Wider tree — show more of each name
```

Truncation replaces the middle of the name with `...`:
```
very_long_project_name.rs  →  very_long_pro...name.rs
```

The displayed extension is always preserved. Applies to both files and directories in the tree view.
